  ...
  <div class="lendBook">
    ...
    <div class="search">
      <div class="studentEnter">
        <el-input
          v-model="account"
          placeholder="请识别学生卡或输入学号"
          @blur="getStudentInfo"
          @focus="clearStudentInfo"
        ></el-input>
      </div>
      <div class="booksEnter">
        <el-input
          v-model="bookid"
          placeholder="请输入图书编号或扫描条形码"
          ref="bookid"
          @keyup.native="checkEnter"
        ></el-input>
...
export default {
  ...
  methods: {
    getStudentInfo() {
      this.$axios
        .get('/admin/getstudentinfo', { params: { account: this.account } })
        .then((res) => {
    ...
    submitLend() {
      ...
      this.$axios
        .get('/admin/lendbooks', { params: { userid: this.studentInfo.id, bookid: this.bookid } })
        .then((res) => {

  ...
  data() {
    return {
      ...
      rules: {
        ...
        bookid: [{ required: true, pattern: /^\S{1,8}\-\d{1,5}$/, message: '图书编号须为xx-xx', trigger: 'blur' }],
        price: [{ required: true, pattern: /^\d{1,6}\.\d{2}$/, message: '价格须为x.xx', trigger: 'blur' }],
      },
    }
  },
  methods: {
    ...
    sendBookData() {
      ...
      this.$axios
        .post('/admin/addbooks', formData1, { headers: { 'Content-Type': 'multipart/form-data' } })

<template>
  <div class="mylend">
    ...
    <div class="lendHis">
      借阅信息 <span @click="getMyLend">刷新<i class="el-icon-refresh-right"></i></span>
    </div>
    <div class="lendBox">
      <div class="booksItem" v-for="(item, index) in lendBox" :key="index">
        ...
        <div class="bth">
          ...
          <el-button type="primary" @click="returnBook(item.bookid)">还书</el-button>
        </div>
      </div>
    </div>
  </div>
...
export default {
  ...
  methods: {
    ...
    renewal(index) {
      ...
      this.addReturnTime(this.lendBox[index].id, newdate)
    },
    returnBook(bookid) {
      this.$axios.get('/books/bookreturn', { params: { bookid } }).then((res) => {
        if (res && (res.data.msg == '还书成功' || res.data.msg == '还书成功，该书已逾期')) {
          ...
          this.lendBox.shift(books) // this.getMyLend()
        }
      })
    },
  },
